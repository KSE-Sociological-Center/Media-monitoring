...
path <- "./df2026 Q1.xlsx"
monitoring_df <- as.data.table(read_excel(path, sheet = "Raw Data"))[
  `Тип медіа` == "web"]

# drop unused cols, clean dates
monitoring_df[, `Дата` := as.Date(`Дата`)]
monitoring_df[, month_year := format(`Дата`, "%Y-%m")]
...
cat("▶ Step 1: визначення релевантних повідомлень...\n")
n_step1 <- nrow(monitoring_df)
with_progress({
  p1 <- progressor(steps = n_step1)
  step1_results <- future_lapply(seq_len(n_step1), function(idx) {
    p1(message = sprintf("Step 1 | %d / %d", idx, n_step1))
    txt <- monitoring_df$`Дайджест тексту`[idx]
    ...
    if (clean == "так") "Так" else if (clean == "ні") "Ні" else NA_character_
  })
})
monitoring_df[, survey_focus := unlist(step1_results)]
...
# 3) Run in parallel
n <- nrow(filtered_df)
cat("▶ Step 2: витягування організацій та теми опитування...\n")
with_progress({
  p2 <- progressor(steps = n)
results <- future_lapply(seq_len(n), function(i) {
  p2(message = sprintf("Step 2 | %d / %d", i, n))
  text <- filtered_df$`Дайджест тексту`[i]
...
cat("▶ Step 4.2: порівняння та нормалізація назв організацій...\n")
n_pairs <- length(grouped_names) - 1

if (n_pairs < 1) {
  message("ℹ️ Step 4.2: менше 2 назв для порівняння — крок пропущено.")
} else {
  pb42 <- txtProgressBar(min = 0, max = n_pairs, style = 3)
  for (i in seq_len(n_pairs)) {
    setTxtProgressBar(pb42, i)
  ...
  if (response == next_name) {
    ...
    # cat(i, " — без змін\n")  # закоментовано щоб не засмічувати прогрес бар
  }
  }
  close(pb42)
  cat("\n")
...
top_150_names <- freq_table[seq_len(min(150, nrow(freq_table))), org_name]


prompt <- paste0(
  "Ось список ", length(top_150_names), " назв організацій, впорядкованих за частотою:\n",
...
texts <- filtered_df$`Дайджест тексту`
...
# Run GPT
cat("▶ Step 5: оцінка якості звітування (AAPOR)...\n")
n_step5 <- length(texts)
with_progress({
  p5 <- progressor(steps = n_step5)
results <- future_lapply(seq_len(n_step5), function(idx) {
  p5(message = sprintf("Step 5 | %d / %d", idx, n_step5))
  txt <- texts[idx]
...
cat("▶ Step 6.1: класифікація ролі опитування (Основне / Згадка)...\n")
with_progress({
  p <- progressor(steps = n)
  results_stype <- future_lapply(seq_len(n), function(i) {
    p(message = sprintf("Step 6.1 | %d / %d", i, n))
    text <- filtered_df$`Дайджест тексту`[i]
...
cat("▶ Step 7.1: тестова класифікація на 10 рядках...\n")

test_indices <- sample(seq_len(nrow(filtered_df)), 10)
with_progress({
  p7test <- progressor(steps = length(test_indices))
  test_results <- future_lapply(seq_along(test_indices), function(j) {
    p7test(message = sprintf("Step 7.1 (тест) | %d / %d", j, length(test_indices)))
    classify_topic(filtered_df$survey_topic[test_indices[j]])
  })
})
...
cat("▶ Step 7.2: повна тематична класифікація...\n")
n_step7 <- nrow(filtered_df)
with_progress({
  p7 <- progressor(steps = n_step7)
  step7_results <- future_lapply(seq_len(n_step7), function(i) {
    p7(message = sprintf("Step 7.2 | %d / %d", i, n_step7))
    classify_topic(filtered_df$survey_topic[i])
  })
})

filtered_df[, classified_topic := unlist(step7_results)]
...
# Step 8: Ідентифікація висвітлення однакових опитувань
filtered_df[, survey_id := NA_character_]

# Підготовка дати
filtered_df[, pub_date := as.Date(substr(Дата, 1, 10))]

# Початковий лічильник — максимальний існуючий survey_id
start_id <- ifelse(
  any(!is.na(filtered_df$survey_id)),
  suppressWarnings(max(as.integer(gsub("\\D", "", filtered_df$survey_id)), na.rm = TRUE)),
  0
)
counter <- start_id
...
for (i in seq_len(nrow(filtered_df))) {
  ...
  if (!is.na(filtered_df$survey_id[i])) next
  
  row_i <- filtered_df[i]
  
  # умови пошуку однакових опитувань
  matches <- which(
    is.na(filtered_df$survey_id) &
      filtered_df$classified_topic == row_i$classified_topic &
      abs(as.numeric(filtered_df$pub_date - row_i$pub_date)) <= 5 &
      (
        (!is.na(filtered_df$org1_final) & filtered_df$org1_final %in% c(row_i$org1_final, row_i$org2_final)) |
          (!is.na(filtered_df$org2_final) & filtered_df$org2_final %in% c(row_i$org1_final, row_i$org2_final))
      )
  )
  ...
  if (length(matches) > 1) {
    counter <- counter + 1
    filtered_df[matches, survey_id := paste0("S", counter)]
  }
}

filtered_df[, pub_date := NULL] #прибираємо технічну колонку

# Огляд результату
filtered_df[!is.na(survey_id), .N, by = survey_id][order(-N)]

# Save
save_partial_results(filtered_df, prefix = "step8")
...
# 1 TOP ORG
# 1.1 Об'єднання двох змінних з організаціями
org_final <- rbind(
  filtered_df[!is.na(org1_final), .(org = org1_final)],
  filtered_df[!is.na(org2_final), .(org = org2_final)]
)

# 1.2 Підрахунок кількості згадок та формування ТОП-20
top20 <- org_final[
  ,
  .N,
  by = org
  ...
  ,
  proportion := N / sum(N)
][order(-proportion)][1:20]

# 1.3 Побудова графіка
org_plot <- ggplot(top20, aes(x = reorder(org, proportion), y = proportion)) +
  ...
  labs(
    title = "Частота згадок організацій у 1-му кварталі 2026 року",
    x     = NULL,
    y     = "Частка згадок (%)"
  ) +
  theme_classic() +
...
# 2 Quality of presentation of survey results
# 2.1 Підрахунок частки
meta_yes <- sapply(meta_keys, function(col) {
  mean(filtered_df[[col]] == "yes", na.rm = TRUE)  
}) %>%
  enframe(name = "meta_key", value = "proportion") %>%
  mutate(
    label_ua = meta_labels,                      
    label_ua = fct_reorder(label_ua, proportion)
  )


# 2.2 Побудова графіка
meta_plot <- ggplot(meta_yes, aes(x = proportion, y = label_ua)) +
  ...
  labs(
    title = "Наявність методологічної інформації в медіапублікаціях у 1-му кварталі 2026 року",
    x     = "Частка новин (%)",
    y     = NULL
  ) +
  theme_classic() +
...
# 2.3 Збереження
ggsave(
  filename = "presentation_quality.png",
  plot     = meta_plot,
  width    = 12,
  height   = 8,
  dpi      = 300
...
# 3.2 Порядок секторів вручну
compliance_summary$compliance_level <- factor(
  compliance_summary$compliance_level,
  levels = c("високий", "середній", "низький")
)
...
# 4 Classified Topics
# 4.0 Словник перекладу тем
topic_translation <- c(
  "Civic Engagement" = "Громадянська участь",
  "Culture and Identity" = "Культура та ідентичність",
  "Economy and Business Climate" = "Економіка та бізнес-клімат",
...
# 4.1 Підрахунок кількості за темами за увесь період
summary_all <- filtered_df %>%
  count(classified_topic) %>%
  mutate(proportion = n / sum(n))

# 4.2 Переклад тем
summary_all$topic_ua <- topic_translation[as.character(summary_all$classified_topic)]

# 4.3 Відфільтрувати "Інше" та впорядкувати за зростанням
summary_all <- summary_all %>%
  filter(topic_ua != "Інше / невизначено") %>%
  mutate(topic_ua = fct_reorder(topic_ua, proportion))

# 4.4 Палітра
n_colors <- nrow(summary_all)
colors   <- scales::hue_pal()(n_colors)

# 4.5 Побудова графіка
classified_plot_all <- ggplot(
  summary_all,
  aes(x = proportion, y = topic_ua)
) +
  geom_col(fill = "deepskyblue") +
  geom_text(
    aes(label = scales::percent(proportion, accuracy = 0.1)),
  ...
  labs(
    title = "Найпопулярніші теми опитувань у 1-му кварталі 2026 року",
    x = "Частка новин (%)",
    y = NULL
  ) +
  theme_classic() +
...
# 4.6 Збереження
ggsave(
  filename = "classified_topics.png",
  plot     = classified_plot_all,
  width    = 12,
  height   = 8,
  dpi      = 300
)
...
sampled_by_topic %>%
  select(classified_topic, `Дайджест тексту`, URL) %>%
  View()
...
# 5.1 тема
  left_join(
    filtered_df %>% 
      select(survey_id, classified_topic) %>% 
      distinct(),
    by = "survey_id"
  ) %>%

# 5.2 3 випадкові URL для групи
  rowwise() %>%
  mutate(
    urls_sample = list({
      urls <- filtered_df %>%
        filter(.data[["survey_id"]] == survey_id) %>%
        pull(URL) %>%
        na.omit() %>%
        unique()
      sample(urls, size = min(3, length(urls)))
    }),
    URL1 = urls_sample[1],
    URL2 = urls_sample[2],
    URL3 = urls_sample[3]
