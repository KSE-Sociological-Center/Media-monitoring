...
#тест (прибрати)
test_df <- monitoring_df[sample(.N, 200)]
...
# Step 3: Валідація, очищення та нормалізація назв організацій

# Крок 1: словник організацій ------------------------------------------------
...
# # Крок 2: Фільтрація організацій, яких немає в словнику=======================
# 
# # 1. Ask GPT whether each value is a concrete org-name or “ні”
# prompt_org <- 
...
# 
# "Persona: Ти — алгоритм вилучення іменованих сутностей (NER).
# Завдання: Проскануй текст і витягни назву установи, організації або медіа-видання, якщо вона згадується.
# Формат: Виведи лише саму назву (без лапок і пояснень). Якщо назви немає — напиши слово 'ні'."
# 
# 
# # Викликаємо GPT тільки для NA у org1_final та org2_final
# for(i in 1:2) {
#   final_col <- paste0("org", i, "_final")
#   clean_col <- paste0("org", i, "_clean")
#   
#   filtered_df[is.na(get(final_col)), (clean_col) := unlist(future_lapply(get(paste0("org", i)), function(txt) {
#     if (is.na(txt) || !nzchar(txt)) return(NA_character_)
#     res <- request_with_retry(paste(prompt_org, txt, sep = "\n\n"), "gpt-4o-mini")
#     str_trim(res)
#   }))]
# 
# # 2. Basic text after GPT
#   filtered_df[, (clean_col) := tolower(get(clean_col))]
#   filtered_df[, (clean_col) := str_replace_all(get(clean_col), '[“”"«».,<>]', '')]
#   filtered_df[, (clean_col) := str_trim(get(clean_col))]
#   filtered_df[, (clean_col) := str_remove_all(get(clean_col), '\\s+(україни|україни)$')]
#   filtered_df[, (clean_col) := str_replace_all(get(clean_col), '[-/]', ' ')]
# }
# 
# #filtered_df %>% select(organisation, org1, org2, org1_final, org2_final, org1_clean, org2_clean) %>% sample_n(30) %>% view()
# save_partial_results(filtered_df[, .(URL, org1_clean,org2_clean)], prefix = "3")
...
# Step 4: Групування подібних назв =============================
# # Вибираємо лише оригінальні значення, якщо _final = NA
# org1_vec <- filtered_df[is.na(org1_final) & !is.na(org1) & org1 != ""]$org1
# org2_vec <- filtered_df[is.na(org2_final) & !is.na(org2) & org2 != ""]$org2
# 
# # Об’єднуємо два вектори
# all_orgs <- c(org1_vec, org2_vec)

# 4.1 Об’єднуємо обидві колонки в один вектор без NA
all_orgs <- na.omit(c(filtered_df$org1_clean_1, filtered_df$org2_clean_1))
all_orgs <- all_orgs[!is.na(all_orgs) & all_orgs != "ні"]
...
# створюємо колонки тільки з потрібними значеннями
filtered_df[, org1_clean_1 := fifelse(
  is.na(org1_final) & !is.na(org1), org1, NA_character_)]
filtered_df[org1_clean_1 == "", org1_clean_1 := NA_character_]
...
#filtered_df %>% select(org1, org2, org1_final, org2_final, org1_clean, org2_clean, org1_clean_1, org2_clean_1) %>% sample_n(30) %>% view()


# 4.3: Формування фінального списку назв організацій =============================
# Об’єднуємо обидві колонки в один вектор і рахуємо частоти
allorgs <- c(filtered_df$org1_clean_1, filtered_df$org2_clean_1)
allorgs <- allorgs[!is.na(allorgs) & allorgs != "ні"]
...
filtered_df[, org1_final1 := {
  can <- mapping[J(org1_clean_1), canonical]
  fifelse(is.na(can), org1_clean_1, can)
}]
filtered_df[, org2_final1 := {
  can <- mapping[J(org2_clean_1), canonical]
  fifelse(is.na(can), org2_clean_1, can)
}]


# Для всіх рядків, де після словника в org1_final та org2_final вийшло NA 
# присвоюємо значення org1_final1 та org2_final1 відповідно
filtered_df[is.na(org1_final), org1_final := org1_final1]
filtered_df[is.na(org2_final), org2_final := org2_final1]

#уся таблиця організацій ДО перетворень
#filtered_df %>% select(organisation, org1, org2, org1_final, org2_final, org1_final1, org2_final1, org1_clean_1, org2_clean_1) %>% sample_n(30) %>% view()
...
# Прибираємо "ні" з org2_final, якщо org1_final та org2_final == "ні"
filtered_df[org1_final == "ні" & org2_final == "ні", org2_final := NA]
# Прибираємо "ні" з тієї змінної, де є організація в іншій
filtered_df[org1_final != "ні" & !is.na(org1_final) & org2_final == "ні", org2_final := NA]
filtered_df[org2_final != "ні" & !is.na(org2_final) & org1_final == "ні", org1_final := NA]
...
filtered_df %>% select(organisation, org1, org2, org1_final, org2_final, org1_final1, org2_final1, org1_clean, org2_clean, org1_clean_1, org2_clean_1) %>% sample_n(30) %>% view()
...
#   paste0(
#     "Для кожного з наведених пунктів напиши 'так' або 'ні' — чи згадується ця інформація в повідомленні. Поверни рівно 9 рядків у форматі:\n",
...
# Step 9: 
# Крок 9.1: збір посилань з новин із фільтром за словником ---------------------------
...
save_partial_results(filtered_df, "check.xlsx")
...
# Крок 9.2: Наявність посилання на звіт/прес-реліз ----------------------------------
...
# Крок 10.1 ідентифікація однакових опитувань через посилання ------------------------
...
# Крок 10.2 Ідентифікація однакових через умови -------------------------------
